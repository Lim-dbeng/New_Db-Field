-- 프로젝트 관리자 관련 테이블 DDL (대체 버전 - 스키마 이름 없이)
...
-- 6. 권한 신청 테이블의 reviewed_by가 프로젝트 관리자인지 확인하는 뷰 (선택사항)
...
-- 7. 부서별 최고 관리자 조회 함수 (각 부서당 1명만 TRUE인지 확인) - 스키마 이름 없이
CREATE OR REPLACE FUNCTION check_dept_admin_unique()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $func$
DECLARE
    admin_count INTEGER;
BEGIN
    -- 같은 부서에 이미 최고 관리자가 있는지 확인 (자기 자신 제외)
...
CREATE TRIGGER trigger_check_dept_admin_unique
    ...
    EXECUTE FUNCTION check_dept_admin_unique();
